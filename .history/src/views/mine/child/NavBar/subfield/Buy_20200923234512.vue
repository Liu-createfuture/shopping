<template>
  <div class="Buy">
    ...
    <van-swipe-cell v-for="(item,index) in GoodsList" :key="index">
      ...
      />
      <template #right>
        <van-button square text="删除" type="danger" class="delete-button" @click="deleteClick(item)" />
      </template>
    </van-swipe-cell>
    <!-- 空状态 -->
...
export default {
  ...
  methods: {
    ...
    a(p) {
      for (let i in this.$store.state.submitList) {
        if (this.$store.state.submitList[i] === p) {
          this.$store.state.submitList.splice(i, 1);
        }
      }
    },
  },
