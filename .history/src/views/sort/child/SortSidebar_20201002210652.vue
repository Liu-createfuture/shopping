...
export default {
  ...
  methods: {
    onChange(item, index) {
      if (
        event.currentTarget.offsetTop > 385 &&
        event.currentTarget.offsetTop <= 550
      ) {
        this.$refs.scroll.scrollTo(0, -54 * (index - 7));
      } else if (event.currentTarget.offsetTop > 550) {
        event.currentTarget.scrollTo(0, -event.currentTarget.offsetTop);
      }
