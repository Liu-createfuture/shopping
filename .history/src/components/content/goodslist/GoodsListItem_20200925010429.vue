...
export default {
  ...
  methods: {
    itemClick() {
      var res = [];
      var obj = [];
      for (var i = 0; i < this.goodsItem.lenth; i++) {
        if (!obj[this.goodsItem[i]]) {
          obj[this.goodsItem[i]] = 1;
          this.$store.state.save.splice(0, 0, this.goodsItem);
          console.log(this.$store.state.save);
        }
      }
      //点击时添加路由,跳转到另外一个页面
