  ...
  [STOCK_COUNT](state, iid) {
    ...
    for (let i in state.submitList) {
      if (state.submitList[i].selectedSkuComb.id === iid) {
        console.log(state.submitList[i].selectedSkuComb.id, iid);
        sum += state.submitList[i].selectedNum;
        state.stockCount = 100 - sum;
        console.log(state.stockCount);
      }
    }
    state.stockCount = 100;
  },
  //历史记录
