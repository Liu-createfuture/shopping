    ...
    [STOCK_COUNT](state, iid) {
        for (let i in state.submitList) {
            if (state.submitList[i].selectedSkuComb.id === iid) {
                //保存数量
                state.stockCount = 100 - state.submitList.reduce((preValue, item) => {
        ...
        for (let i in state.cartList) {
            if (state.cartList[i].selectedSkuComb.id === iid) {
                //保存数量
                console.log(state.cartList[i].selectedSkuComb.id === iid);
                state.stockCount = 100 - state.cartList.reduce((preValue, item) => {
                    return preValue + item.selectedNum
                }, 0)
            }
        }
