...
export default {
    ...
    [STOCK_COUNT](state, iid) {
        ...
        for (let i in state.submitList) {
            if (state.submitList[i].selectedSkuComb.id === iid) {
                state.submitList.reduce((preValue, item) => {
                    //合计
                    return preValue + selectedSkuComb.stock_num;
                  }, 0);
                state.submitList[i].selectedSkuComb.stock_num -= state.submitList[i].selectedSkuComb.stock_num - state.submitList[i].selectedNum
                console.log(state.submitList[i].selectedSkuComb.stock_num);
            }

            //保存数量
            // state.stockCount = 100 - state.cartList[i].selectedNum;
